...
@section('main')
    ...
    <div class='px-12 py-3 text-sm'>
        ...
        <h1 class='text-xl mt-2 mb-6'>status</h1>

        <form method="POST" action="/admin/status"  class='flex flex-col gap-3 min-w-[25rem]'>
            @method('PATCH')
            @csrf
            ...
            <div class='flex flex-col gap-2'>
                ...
                <div>
                    <input type="name" id="name" name="name" class='w-full p-2 outline-none rounded-md  border'  value="{{$status->name}}" required />
                    @error('name')
