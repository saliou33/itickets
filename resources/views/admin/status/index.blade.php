...
@section('main')
    ...
    <div class='px-12 py-5 text-sm'>
        <h1 class='text-xl mb-6'>Etats</h1>
        <table class='table-auto font-serif'>
            <thead class='border-b'>
                <tr class='bg-gray-100 text-[1.1rem]'>
                    <td class='px-8 py-3 rounded-tl-2xl'>Id</td>
                    <td class='px-8 py-3'>Nom</td>
            ...
            <tbody>
                @foreach($statuses as $status)
                    <tr class='border-b h-4'>
                        <td class='px-8 py-2'>{{$status->id}}</td>
                        <td class='px-8 py-2'>{{$status->name}}</td>
                        <td class='px-8 py-2'>{{$status->created_at}}</td>
                        <td class='px-8 py-2'>{{$status->updated_at}}</td>
                        <td class='flex justify-center items-center gap-3'>

                            <button class='py-2'>
                                <a href="/admin/status/{{$status->id}}">
    ...
    <div class="">
        ...
        <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="exampleModalCenteredScrollable" tabindex="-1" aria-labelledby="exampleModalCenteredScrollable" aria-modal="true" status="dialog">
